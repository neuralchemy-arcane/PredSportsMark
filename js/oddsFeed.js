/* Sports Betting Odds API adapter (free BASIC: 100 req/month).
   One cached snapshot per day; rows matched to fixtures by fuzzy team names.
   Defensive parser: handles outcome arrays, flat *moneyline/*odds keys,
   and accidental American odds. Use the Probe button to inspect raw rows. */
...
const DPOddsFeed = {
  ...
  toDecimal(v) {
    if (v < 0) return 1 + 100 / Math.abs(v);   // American negative
    if (v > 15) return 1 + v / 100;             // American positive
